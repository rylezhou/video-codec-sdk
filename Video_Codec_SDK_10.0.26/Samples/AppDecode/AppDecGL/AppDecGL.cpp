...
int Decode(CUcontext cuContext, char *szInFilePath) {
    ...
    int nPitch = demuxer.GetWidth()*4;

#if defined (_WIN32)
    FramePresenterGLUT gInstance(cuContext, demuxer.GetWidth(), demuxer.GetHeight());
#else
    FramePresenterGLX gInstance(demuxer.GetWidth(), demuxer.GetHeight());
#endif
