        ...
        for (int i = 0; i < nFrameReturned; i++)
        {
            ...
            if (dec.GetBitDepth() == 8)
            {
                 if (dec.GetOutputFormat() == cudaVideoSurfaceFormat_YUV444)
                    YUV444ToColor32<BGRA32>(pFrame, dec.GetWidth(), (uint8_t *)dpFrame, 4 * dec.GetWidth(), dec.GetWidth(), dec.GetHeight(), iMatrix);
                else    // default assumed as NV12
                    Nv12ToColor32<BGRA32>(pFrame, dec.GetWidth(), (uint8_t *)dpFrame, 4 * dec.GetWidth(), dec.GetWidth(), dec.GetHeight(), iMatrix);
            }
            else
            {
                if(dec.GetOutputFormat() == cudaVideoSurfaceFormat_YUV444_16Bit)
                    YUV444P16ToColor32<BGRA32>(pFrame, 2 * dec.GetWidth(), (uint8_t *)dpFrame, 4 * dec.GetWidth(), dec.GetWidth(), dec.GetHeight(), iMatrix);
                else // default assumed as P016
                    P016ToColor32<BGRA32>(pFrame, 2 * dec.GetWidth(), (uint8_t *)dpFrame, 4 * dec.GetWidth(), dec.GetWidth(), dec.GetHeight(), iMatrix);
            }
